...
M.profiles = {
  node = {
    markers = { "package.json", "tsconfig.json", "jsconfig.json", "pnpm-workspace.yaml", "yarn.lock", "pnpm-lock.yaml" },
    servers = { "ts_ls", "eslint", "tailwindcss", "html", "cssls", "jsonls" },
  },
  python = {
    markers = { "pyproject.toml", "requirements.txt", "setup.py", "Pipfile", "environment.yml", "pyrightconfig.json" },
    servers = { "pyright" },
  },
  rust = {
...
function M.try_spawn(server_name, bufnr)
  ...
  if not allowed[server_name] then return end
    ...
    if entry and (now - state.last_used) > entry.timeout then
      ...
      for _, client in ipairs(clients) do
        ...
        for _, bufnr in ipairs(attached_bufs) do
          ...
          if any_visible then break end
        end

        if not any_visible then
          vim.lsp.stop_client(client.id, true)
          vim.notify(("[LSP] 💤 %s stopped (idle %ds)"):format(server_name, now - state.last_used), vim.log.levels.INFO)
...
timer:start(reaper_interval * 1000, reaper_interval * 1000, vim.schedule_wrap(function()
  M.reap_idle()
end))
