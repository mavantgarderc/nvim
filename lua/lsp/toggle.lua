...
function M.toggle_current_filetype()
  ...
  if #to_toggle == 0 then
    vim.notify(
      string.format("[LSP Dynamic] No registered servers for filetype '%s'", ft),
      vim.log.levels.WARN
    )
    return
  end
  ...
  if any_on then
    ...
    vim.notify(
      string.format("[LSP Dynamic] Disabled LSP(s) for filetype '%s'", ft),
      vim.log.levels.INFO
    )
  else
    for _, server_name in ipairs(to_toggle) do
    ...
    vim.notify(
      string.format("[LSP Dynamic] Enabled LSP(s) for filetype '%s'", ft),
      vim.log.levels.INFO
    )
  end
end
