...
local function check_version()
  ...
  return is_compatible, string.format(
...
local function check_plugins()
  ...
  if #missing_plugins == 0 then
    ...
    table.insert(results, {
      ok = false,
      message = string.format("Missing %d/%d plugins: %s",
        #missing_plugins,
...
local function check_lsp_servers()
  ...
  table.insert(results, {
    ok = true,
    message = string.format("LSP servers available: %d/%d",
      #available_servers,
