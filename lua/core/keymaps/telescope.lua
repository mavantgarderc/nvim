  ...
  callback = function(event)
    if event.data == "telescope.nvim" then
      ...
      map(
        "n",
        "<leader><leader>",
        function() builtin.buffers({ sort_mru = true, sort_lastused = true, initial_mode = "normal" }) end,
      ...
      map("n", "<leader>fF", function() builtin.find_files({ hidden = true }) end, { desc = "Find files (including hidden)" })
      ...
      map(
        "n",
        "<leader>s/",
        function() builtin.live_grep({ prompt_title = "Live Grep in Open Files" }) end,
      ...
      map("n", "<leader>fp", function() builtin.find_files({ cwd = "~/projects" }) end, { desc = "Find project files" })
    end
  end,
})
